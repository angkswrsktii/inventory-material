@extends('layouts.app')
@section('title', __('app.good_issue.title'))
...
<div class="page-header">
    <div>
        ...
        <div class="page-subtitle">{{ __('app.good_issue.detail_subtitle') }}</div>
    </div>
...
<div style="display:grid; grid-template-columns: 320px 1fr; gap:20px; align-items:start;">
    ...
    <div class="card">
        <div class="card-header">
            <span class="card-title"><i class="fas fa-info-circle" style="color:var(--accent);margin-right:8px;"></i>{{ __('app.good_issue.info_title') }}</span>
        </div>
        <div style="padding:0;">
            @php
                $rows = [
                    [__('app.good_issue.no_gi'),           $goodIssue->gi_number],
                    [__('app.good_issue.production_target'),$goodIssue->part->part_name ?? '-'],
                    [__('app.good_issue.pic_cutter'),       $goodIssue->pic->name ?? '-'],
                    [__('app.common.project'),              $goodIssue->project->name ?? '-'],
                    [__('app.good_issue.issue_date'),       $goodIssue->issue_date->format('d M Y')],
                    [__('app.good_issue.issued_by'),        $goodIssue->issuer->name ?? '-'],
                ];
            @endphp
            ...
            <div style="padding:12px 20px;">
                <div style="font-size:12px; color:var(--text-muted); margin-bottom:5px;">{{ __('app.good_issue.purpose_notes') }}</div>
    ...
    <div class="card">
        <div class="card-header">
            <span class="card-title"><i class="fas fa-boxes-stacked" style="color:var(--accent-2);margin-right:8px;"></i>{{ __('app.good_issue.items_title') }}</span>
        </div>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th>{{ __('app.good_issue.col_load_number') }}</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $goodIssue->warehouse->name ?? '-' }}</td>

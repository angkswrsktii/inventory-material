...
@section('topbar-title', 'Edit Purchase Request')

@section('content')
<div class="breadcrumb">
    <a href="{{ route('purchase-requests.index') }}">Purchase Request</a>
    <span class="sep">/</span>
    <span>Edit: {{ $purchaseRequest->pr_number }}</span>
</div>

<div class="page-header">
    <div>
        <div class="page-title">Edit Purchase Request</div>
        <div class="page-subtitle">Perbarui data permintaan pembelian material</div>
    </div>
</div>
            ...
            <div class="card-body">
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="pr_number" class="form-control" value="{{ old('pr_number', $purchaseRequest->pr_number) }}" readonly style="background:var(--surface-2);">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Tanggal PR <span class="required">*</span></label>
                        <input type="date" name="request_date" class="form-control" value="{{ old('request_date', $purchaseRequest->request_date->format('Y-m-d')) }}" required>
            ...
            <div class="table-wrap">
                <table id="itemsTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <select name="items[{{ $index }}][m_material_id]" class="form-control" onchange="updateRow(this)" required>
                                    <option value="">-- Pilih Material --</option>
                                    @foreach($materials as $material)
                                        <option value="{{ $material->id }}" data-unit="{{ $material->unit }}" {{ $item->m_material_id == $material->id ? 'selected' : '' }}>
                            ...
                            <td>
                                <div style="display:flex;align-items:center;gap:8px;">
                                    <input type="number" name="items[{{ $index }}][quantity]" class="form-control" value="{{ $item->quantity }}" min="0.01" step="0.01" style="font-size:13px;padding:6px;width:100%;" required>
                                    <span class="unit-label" style="font-size:12px;color:var(--text-muted); width:30px;">{{ $item->material->unit ?? '' }}</span>
                                </div>
                            </td>
                            <td>
                                <input type="text" name="items[{{ $index }}][purpose]" class="form-control" value="{{ $item->notes }}" required style="font-size:13px;padding:6px;">
                            </td>

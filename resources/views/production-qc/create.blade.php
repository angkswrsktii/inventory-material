@extends('layouts.app')
@section('title', __('app.production_qc.create_title'))
...
<div class="breadcrumb">
    ...
    <span class="sep">/</span>
    <span>{{ __('app.production_qc.create_breadcrumb') }}</span>
</div>

<div class="page-header">
    <div>
        <div class="page-title">{{ __('app.production_qc.create_title') }}</div>
        <div class="page-subtitle">{{ __('app.production_qc.create_subtitle') }}</div>
    </div>
</div>
    ...
    <div class="card" style="margin-bottom:20px;">
        <div class="card-body">
            <form method="GET" action="{{ route('production-qc.create') }}">
                <label class="form-label" style="font-weight:600;">{{ __('app.production_qc.select_gi') }} <span class="required">*</span></label>
                <div style="display:flex; gap:10px;">
                    <select name="t_good_issue_id" class="form-control" onchange="this.form.submit()" required>
                        <option value="">{{ __('app.production_qc.select_gi_ph') }}</option>
                        @foreach($availableGIs as $gi)
                            <option value="{{ $gi->id }}" {{ request('t_good_issue_id') == $gi->id ? 'selected' : '' }}>
                                {{ $gi->gi_number }} | {{ __('app.production_qc.production_target') }}: {{ $gi->part->part_name ?? '-' }} | {{ __('app.common.pic') }}: {{ $gi->pic->name ?? '-' }}
                            </option>
                        @endforeach
                    </select>
                </div>
    ...
    <form action="{{ route('production-qc.store') }}" method="POST">
        ...
        <div class="card" style="margin-bottom:20px;">
            <div class="card-header">
                <span class="card-title"><i class="fas fa-link" style="color:var(--accent);margin-right:8px;"></i>{{ __('app.production_qc.from_gi') }}</span>
            ...
            <div class="card-body">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
                    ...
                    <div>
                        <div style="font-size:11px;color:var(--text-dim);text-transform:uppercase;">{{ __('app.production_qc.pic_cutter') }}</div>
                    ...
                    <div>
                        <div style="font-size:11px;color:var(--text-dim);text-transform:uppercase;">{{ __('app.good_issue.issue_date') }}</div>
                        <div style="margin-top:3px;color:var(--text-muted);">{{ $goodIssue->issue_date->format('d M Y') }}</div>
                    </div>
                </div>

                <div style="font-size:12px;color:var(--text-dim);margin-bottom:8px;text-transform:uppercase;letter-spacing:.5px;">{{ __('app.production_qc.issued_materials') }}</div>
        ...
        <div class="card" style="margin-bottom:20px;">
            <div class="card-header">
                <span class="card-title"><i class="fas fa-clipboard-check" style="color:var(--accent-2);margin-right:8px;"></i>{{ __('app.production_qc.qc_result_title') }}</span>
            ...
            <div class="card-body">
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:14px;">
                        ...
                        <div class="form-group" style="margin-bottom:0;">
                            <label class="form-label" style="color:var(--success); font-weight:600;">
                                <i class="fas fa-check-circle"></i> {{ __('app.production_qc.ok_label') }} <span class="required">*</span>
                            </label>
                            <div style="display:flex; gap:8px; align-items:center;">
                                <input type="number" name="quantity_passed" id="qtyPassed" class="form-control"
                                       value="{{ old('quantity_passed', 0) }}" min="0" step="0.01" required
                        ...
                        <div class="form-group" style="margin-bottom:0;">
                            <label class="form-label" style="color:var(--danger); font-weight:600;">
                                <i class="fas fa-times-circle"></i> {{ __('app.production_qc.ng_scrap') }} <span class="required">*</span>
                            </label>
                            <div style="display:flex; gap:8px; align-items:center;">
                                <input type="number" name="quantity_failed" id="qtyFailed" class="form-control"
                                       value="{{ old('quantity_failed', 0) }}" min="0" step="0.01" required oninput="calculateTotalNG()"
                ...
                <div class="form-group" style="margin-top:14px;">
                    ...
                    <textarea name="notes" class="form-control" rows="3" placeholder="{{ __('app.production_qc.notes_placeholder') }}">{{ old('notes') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div style="display:flex;gap:10px;">
            ...
            <a href="{{ route('production-qc.index') }}" class="btn btn-ghost">
                <i class="fas fa-times"></i> {{ __('app.btn.cancel') }}
            </a>
        </div>
    </form>
